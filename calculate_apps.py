...
data_path = f"/home/jzchen/ML/Code/data/apps_simple.jsonl.gz"
exp_name = sys.argv[1]
...
for task_id in range(int(start_idx), int(end_idx) + 1):
    A_path = os.path.join(
        root, f"{exp_name}", f"{model_A_name}_{convert(task_id)}_{item}.json"
    )
    B_path = os.path.join(
        root, f"{exp_name}", f"{model_B_name}_{convert(task_id)}_{item}.json"
    )
    if not os.path.exists(A_path):
        A_path = os.path.join(
            root, f"{exp_name}", f"{model_A_name}_{convert(task_id)}_final.json"
        )
    if not os.path.exists(B_path):
        B_path = os.path.join(
            root, f"{exp_name}", f"{model_B_name}_{convert(task_id)}_final.json"
        )
    result_A.update(json.load(open(A_path, "r")))
    result_B.update(json.load(open(B_path, "r")))

result_A = {convert(k): v for k, v in result_A.items()}
result_B = {convert(k): v for k, v in result_B.items()}
print(len(result_A))
