...
def run_test(
    ...
    if os.path.exists(os.path.join(root, "input_output.json")):
        ...
        in_outs = {
            "inputs": problem["example_testcases"]["input"]
            + problem["testcases"]["input"],
            "outputs": problem["example_testcases"]["output"]
            + problem["testcases"]["output"],
        }
    else:
        print(problem)
        print("No valid test case is provided.")
        exit()
    if check_part == "first":
        in_outs["inputs"] = in_outs["inputs"][: len(in_outs["inputs"]) // 2]
        in_outs["outputs"] = in_outs["outputs"][: len(in_outs["outputs"]) // 2]
    if check_part == "last":
        in_outs["inputs"] = in_outs["inputs"][len(in_outs["inputs"]) // 2 :]
        in_outs["outputs"] = in_outs["outputs"][len(in_outs["outputs"]) // 2 :]
    if debug:
